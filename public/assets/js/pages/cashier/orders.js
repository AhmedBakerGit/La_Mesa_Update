...
var baseOrderUrl = window.location.origin + '/ajax/cashier/';
...
async function cancelOrder(cObj) {
  var orderID = $(cObj).data('ref');

  var modal = $('#modalCancelOrder');

  modal.find(".modal-title").html("You need to prepare manager info");
  modal.find('input[name="orderId"]').val(orderID);
  modal.find('label[name="orderId"]').html(orderID);
...
$('#modalCancelOrder button').on('click', function() {
  var modal = $('#modalCancelOrder');

  var orderID = modal.find('label[name="orderId"]').html();
  var name = modal.find('input[name="name"]').val();
  var password = modal.find('input[name="password"]').val();
  
  if (orderID == "" || name == "" || password == "") {
  ...
  apiComm.PostApiCall(baseOrderUrl+'checkManagerCredentials', 'CancelResult', params);
})

async function CancelResult(res) {
  var modal = $('#modalCancelOrder');
