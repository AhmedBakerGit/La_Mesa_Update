...
var baseUrl = window.location.origin + '/ajax/admin/staff/';
...
var dtStaff = {
  ...
  editStaff: function(res) {
    if (res.status == 1) {
      ...
      modal.find("select[name='restaurant']").val(res.data.restaurant_id);
  ...
  submit: function() {
    ...
    if (!modal.find("input[name='f_name']").val() || !modal.find("input[name='l_name']").val() || !modal.find("input[name='password']").val() ||
      !modal.find("select[name='restaurant']").val() || !modal.find("select[name='role']").val()) {
